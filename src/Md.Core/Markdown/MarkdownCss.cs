...
/// <summary>
/// md's own stylesheet, byte for byte — the port of <c>MarkdownHTML.css(dark:export:)</c>, one
/// literal with seven interpolations and the same 101 lines in every variant.
/// </summary>
/// <remarks>
/// <para>
/// Built as a <c>string[]</c> joined with <c>"\n"</c>, never as a raw literal: a raw literal takes
/// its newlines from the <c>.cs</c> file, and a checkout with CRLF would emit <c>\r\n</c> and fail
/// every stylesheet golden silently. No trailing newline — the Swift literal ends at the last rule
/// and <c>document()</c> glues <c>&lt;/style&gt;</c> straight onto it.
/// </para>
/// <para>
/// Three reachable sheets, not four: <c>dark &amp;&amp; !export</c> runs in <c>Document</c> before
/// the palette is computed and is repeated here, so a caller reaching <see cref="Stylesheet"/>
/// directly cannot produce the never-shipped dark-ink-on-white variant no golden covers.
/// </para>
/// <para>
/// Hazards the shipping tests pin: every <c>/*</c> has exactly one <c>*/</c> (a comment that closes
/// early makes the CSS parser swallow the next rule whole, and the Graphviz ink rules sit under the
/// longest one); the blank line inside the Graphviz comment is intentional; line 32 is genuinely
/// blank in the preview variants (the empty <c>preWrap</c> interpolation), leaving a comment that
/// describes a rule which is not there; <c>\newpage</c> is one backslash in the output; the only
/// indentation is 4 spaces inside <c>body { … }</c> and the rich-block rule and 3 spaces continuing
/// each comment. Kotlin's sheet differs (font, hljs rules, string tone) and is not a reference.
/// </para>
/// </remarks>
internal static class MarkdownCss
{
    public static string Stylesheet(bool dark, bool export)
    {
        var isDark = dark && !export;

        // The first four come from the app's asset catalog (PaperBackground, PaperInk,
        // PaperBackgroundSecondary, AccentColor); the rest exist only in CSS.
        var paper = isDark ? "#241E18" : "#F4EFE2";
        var ink = isDark ? "#E7DBC2" : "#2B2620";
        var secondary = isDark ? "#2F2820" : "#EAE2CF";
        var accent = isDark ? "#C99A55" : "#9C6B2E";
        var muted = isDark ? "#B3A98E" : "#6B635A";
        // Literally ink at 16 % alpha, emitted as this exact string and never computed: the tests
        // grep for it and the HTML export's page-break swap substitutes it.
        var border = isDark ? "rgba(231,219,194,0.16)" : "rgba(43,38,32,0.16)";
        // The hljs string tone: a shade of the ink, warmer and darker than `muted`. Android ships
        // #6A5433 / #B79A67 here; Apple is the reference.
        var codeString = isDark ? "#CDBF9E" : "#4A4034";

        var preWrap = export ? "pre { white-space: pre-wrap; overflow-wrap: anywhere; }" : "";
        var pageBreak = export
